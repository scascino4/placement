...
[[nodiscard]] Rectangle cell_rectangle(const Cell &cell) {
  ...
  case Orientation::W:
  case Orientation::FE:
  case Orientation::FW: std::swap(width, height); break;
  default: break;
  }
...
[[nodiscard]] CellClass cell_class(const Cell &cell) {
  ...
  if (cell.kind == CellKind::Terminal ||
      cell.location->status == PlacementStatus::Fixed)
...
void write_paths(std::ostream &output, const Board &board, CellClass wanted,
  ...
  for (const auto &cell : board.cells) {
    if (!cell.location || cell_class(cell) != wanted) continue;
    const auto rectangle = cell_rectangle(cell);
    if (rectangle.width == 0 || rectangle.height == 0) continue;
    if (in_path == 0) output << "    <path class=\"" << css_class << "\" d=\"";
    output << 'M' << rectangle.x << ' ' << rectangle.y << 'h' << rectangle.width
           << 'v' << rectangle.height << 'h' << -rectangle.width << 'z';
    ++in_path;
...
public:
  void render(const Board &board, const std::filesystem::path &output_path) const override {
    ...
    for (const auto &row : board.rows) {
      for (const auto &subrow : row.subrows)
        bounds.include({subrow.origin, row.coordinate,
                        static_cast<double>(subrow.site_count) * row.site_spacing,
                        row.height});
    }
    for (const auto &cell : board.cells)
      if (cell.location) bounds.include(cell_rectangle(cell));
    if (bounds.empty()) throw Error("cannot render a board without geometry");
    ...
    try {
      ...
      if (!output) throw Error("cannot create " + output_path.string());
      ...
      output << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
             ...
             << "  <desc>" << board.cells.size() << " cells, " << board.rows.size()
             << " rows, " << board.nets.size() << " nets</desc>\n"
             << "  <style>\n"
             << "    .background{fill:#f8fafc}.row{fill:#e2e8f0;stroke:#94a3b8;stroke-width:"
             << stroke
             ...
             << "}\n"
             << "  </style>\n"
             << "  <rect class=\"background\" x=\"" << -padding << "\" y=\""
             << -padding << "\" width=\"" << width + 2 * padding << "\" height=\""
             << height + 2 * padding << "\"/>\n"
             << "  <g transform=\"translate(" << -bounds.minimum_x << ' '
             << bounds.maximum_y << ") scale(1 -1)\" shape-rendering=\"crispEdges\">\n";
      for (const auto &row : board.rows) {
        for (const auto &subrow : row.subrows) {
          output << "    <rect class=\"row\" x=\"" << subrow.origin << "\" y=\""
                 << row.coordinate << "\" width=\""
                 << static_cast<double>(subrow.site_count) * row.site_spacing
                 << "\" height=\"" << row.height << "\"/>\n";
        }
      }
      ...
      output << "  </g>\n</svg>\n";
      output.flush();
      if (!output) throw Error("failed while writing " + output_path.string());
      output.close();
...
std::unique_ptr<Renderer> make_renderer(std::string_view format) {
  if (lower(format) == "svg") return std::make_unique<SvgWriter>();
